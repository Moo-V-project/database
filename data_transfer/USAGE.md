...
By default the exporter uses Claude Haiku to summarize movie reviews. This requires an Anthropic API key and is a **paid service** (though very cost-effective with Haiku).

If you prefer not to use it, simply remove `reviews_aggregator` from `cli.py` — the exporter will still work, `reviews_sum` column will be empty.

You can also implement your own aggregator — just make sure it has a `summarize_reviews(reviews: list[dict]) -> str` method and pass it to `TMDBExporter`.
