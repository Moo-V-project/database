...
class TestBuildImageUrl:
    def test_returns_full_url(self, mock_exporter):
        assert mock_exporter._build_image_url("/abc.jpg") == "https://image.tmdb.org/t/p/original/abc.jpg"
...
class TestExtractJobFromCharacter:
    ...
    def test_narrator_extracted(self, mock_exporter):
        assert mock_exporter._extract_job_from_character("Narrator (Narrator)") == "Narrator"

    def test_unapproved_job_returns_default(self, mock_exporter):
        assert mock_exporter._extract_job_from_character("Bob (Stuntman)") == DEFAULT_CAST_JOB
...
class TestTransformMovieData:
    ...
    def setup(self, movie_stub, credits_stub):
        ...
        self.mock_exporter = TMDBExporter(self.fetcher, self.mock_reviews_aggregator, output_dir="/tmp/tmdb_test")
    ...
    def test_basic_fields(self):
        ...
        assert self.mock_exporter.transform_movie_data(550)["release_date"] == "1999-10-15"
        assert self.mock_exporter.transform_movie_data(550)["homepage"] == "http://www.foxmovies.com/movies/fight-club"
        assert self.mock_exporter.transform_movie_data(550)["tagline"] == "Mischief. Mayhem. Soap."
        
    def test_zero_values_handling(self):
        stub_with_zero = {"id": 550, "title": "Fight Club", "budget": 0, "revenue": 0, "runtime": 0, "vote_count": 0, "vote_average": 0}
    ...
    def test_poster_url_built(self):
        result = self.mock_exporter.transform_movie_data(550)
        assert result["poster_url"] == "https://image.tmdb.org/t/p/original/pB8BM7pdSp6B6Ih7QZ4DrQ3PmJK.jpg"
    ...
    def test_production_companies(self):
        result = self.mock_exporter.transform_movie_data(550)
        expected_companies = ["Fox 2000 Pictures","Regency Enterprises", "Linson Entertainment", "20th Century Fox", "Taurus Film"]
        assert json.loads(result["companies"]) == expected_companies
        
        stub_with_missing_company_name = {"id": 555, "title": "Gay Club", "production_companies": [{"id": 1}, {"id": 2, "name": "Company Name"}]}
    ...
    def test_genres(self):
        self.fetcher.get_genres.return_value = [{"id": 18, "name": "Drama"}, {"id": 53, "name": "Thriller"}]
        assert json.loads(self.mock_exporter.transform_movie_data(550)["genres"]) == [18, 53]
        
        stub_with_missing_genre_id = {"id": 555, "title": "Gay Club", "genres": [{"name": "Drama"}, {"id": 53, "name": "Thriller"}]}
